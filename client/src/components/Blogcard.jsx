...
const BlogCard = ({ blog, onLikeUpdate }) => {
  ...
  return (
    <article className="bg-white rounded-lg border border-gray-200 hover:shadow-lg transition-all duration-300 overflow-hidden group">
      {/* Featured Image */}
      {blog.featuredImage && (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              onClick={handleLike}
              disabled={isLiking}
              className={`text-gray-500 hover:text-red-500 ${blog.isLiked ? "text-red-500" : ""}`}
            >
              <Heart className={`h-4 w-4 mr-1 ${blog.isLiked ? "fill-current" : ""}`} />
              <span className="text-xs">{blog.likesCount || 0}</span>
            </Button>
            ...
            <Button
              ...
              className={`text-gray-500 hover:text-yellow-500 ${isBookmarked ? "text-yellow-500" : ""}`}
            >
